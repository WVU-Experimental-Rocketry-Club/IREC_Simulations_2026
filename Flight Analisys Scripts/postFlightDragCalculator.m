...
coast_start_delay = 3; % Adjust between 0.25 to 1.0 for best results
coast_end_early = 3;
...
try
    ...
    timeVar = find(strcmpi(data.Properties.VariableNames, 'time'));
    altVar = find(strcmpi(data.Properties.VariableNames, 'altitude_gps'));
    velVar = find(strcmpi(data.Properties.VariableNames, 'speed'));
    accelVar = find(strcmpi(data.Properties.VariableNames, 'accel_x')); % Prioritize accel_x
    pressureVar = find(contains(data.Properties.VariableNames, 'pressure', 'IgnoreCase', true), 1);
    tempVar = find(contains(data.Properties.VariableNames, 'Temperature', 'IgnoreCase', true), 1);


    % If not found, try alternative names
    if isempty(accelVar), accelVar = find(strcmpi(data.Properties.VariableNames, 'Vert-Acc(m/s^2)')); end
    if isempty(timeVar), timeVar = find(contains(data.Properties.VariableNames, 'Time', 'IgnoreCase', true), 1); end
    if isempty(altVar), altVar = find(contains(data.Properties.VariableNames, 'Altitude', 'IgnoreCase', true), 1); end
    if isempty(velVar), velVar = find(contains(data.Properties.VariableNames, 'Vel', 'IgnoreCase', true), 1); end
    if isempty(accelVar), accelVar = find(contains(data.Properties.VariableNames, 'Acc', 'IgnoreCase', true), 1); end
    
    % Add an additional check for the 'speed' column name found in your sample file.
    if isempty(velVar), velVar = find(strcmpi(data.Properties.VariableNames, 'speed'), 1); end
    
    % Check for the new required columns as well
    if any([isempty(timeVar), isempty(altVar), isempty(velVar), isempty(accelVar), isempty(pressureVar), isempty(tempVar)])
        error('Could not automatically detect required columns (Time, Altitude, Velocity, Acceleration, Pressure, Temperature) in the CSV file. Please check column headers.');
    end
    
    time = data{:, timeVar};
    altitude = data{:, altVar};
    velocity = data{:, velVar};
    acceleration = data{:, accelVar};
    pressure = data{:, pressureVar}; % Load pressure data (Pa)
    temperature = data{:, tempVar}; % Load temperature data (C)
...
validIdx = isfinite(time) & isfinite(altitude) & isfinite(velocity) & isfinite(acceleration) & isfinite(pressure) & isfinite(temperature);
time = time(validIdx);
altitude = altitude(validIdx);
velocity = velocity(validIdx);
acceleration = acceleration(validIdx);
pressure = pressure(validIdx);
...
burnoutTime = time(burnoutIdx);

% Find the actual start of the coast phase by adding the delay
coastStartTime = burnoutTime + coast_start_delay;
coastStartIdx = find(time >= coastStartTime, 1);

% If delay pushes us past apogee, handle the error
if isempty(coastStartIdx)
...
coastEndIdx = find(time >= coastEndTime, 1);

% Define the coast phase data range
coastIdx = coastStartIdx:coastEndIdx;
...
fprintf(' - Coast analysis begins at %.2f seconds (%.2fs after max velocity).\n', coastStartTime, coast_start_delay);
...
pressure_coast = pressure(coastIdx);
temp_coast = temperature(coastIdx);
